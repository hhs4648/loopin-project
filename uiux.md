...
| **문장으로 나누기** | 본문에서 **단원 저장 단어**를 찾아 단어 목록에서 선택하고, 그 단어가 나온 **본문 문장·뜻**을 예문(영어)·예문 뜻에 채움 · 표제어의 `~` 는 없어도 됨 (`draw ~ attention to` → `draw attention to` / `drew my attention to`) · `'A'` / `'B'` / `A` / `B` 는 그 자리에 단어가 있어야 함 (`keep A from B` → `keep kids from playing`) · 단어 수정 팝업에도 그대로 보임 · 본문·뜻을 문장으로 나눠 문장 목록에 넣고 선택 · **마침표 뒤 칸이 없어도** 가름 (`A.B.` → `A.` / `B.`) · `3.14` 같은 소수점은 한 덩어리 · 같은 영어 문장이 단원에 있으면 그걸 고름 · 다시 누르면 이전에 만든 `sent-passage-…` 문장만 교체 (이동 없음) |
...
| 단어 **기초단어** 체크 | 유형 선택 오른쪽 · **켜면** 기초로 표시한 단어도 목록에 보임 · **끄면** 기초단어는 숨기고 선택에서도 뺌 · 기본 꺼짐 (이동 없음) |
...
| 본문 표 팝업 · **자동 나눔** (영어/한글 청크 열 머리) | 채워진 줄의 영어·한글을 `/` 청크로 채워 넣음 · 해당 칸 소스가 비어 있으면 안내 (이동 없음) |
| 본문 표 팝업 · **청크 자동 나누기** | 영어·한글이 있는 줄의 청크를 한꺼번에 나눔 · 비어 있으면 안내 (이동 없음) |
...
| 본문 수정 팝업 · **자동 나눔** (영어/한글 청크) | 위 영어·한글 예문을 `/` 청크로 채워 넣음 · 예문이 비어 있으면 안내 (이동 없음) |
