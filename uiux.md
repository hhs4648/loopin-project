...
| **3 본문** | 사용자 지정 과제와 같이 **본문 / 본문 뜻** 두 칸 · 단원 선택 **전**에는 비활성 · **문장으로 나누기** 시 (1) 본문에 나온 **단원 저장 단어**를 고르고, 그 단어가 나온 **본문 문장·뜻**을 예문 칸에 채움(`upsertCustomWord`, 원본 JSON은 유지) — 표제어의 `~` / `'A'` / `'B'` / `A` 자리는 어떤 단어가 들어가도 같은 표현으로 본다 (`keep ~ from ~` → `keep kids from playing`) (2) 본문·뜻을 문장 단위로 나눠 문장 문항으로 넣음(이미 같은 영어 문장이 단원에 있으면 그걸 고르고, 없으면 `haksup-custom-problem-bank`에 `sent-passage-…` id로 추가 · 다시 나누면 이 묶음만 교체) · 영어·한글 문장 수가 다르면 앞에서부터 맞추고 파란 안내 · 적용 후 단어·문장 목록을 펼치고 해당 항목을 **자동 선택** |
...
| **문장으로 나누기** | 본문에서 **단원 저장 단어**를 찾아 단어 목록에서 선택하고, 그 단어가 나온 **본문 문장·뜻**을 예문(영어)·예문 뜻에 채움 · 표제어의 `~` / `'A'` / `'B'` / `A` 자리는 어떤 단어가 들어가도 같은 표현으로 본다 (`keep ~ from ~` → `keep kids from playing`) · 단어 수정 팝업에도 그대로 보임 · 본문·뜻을 문장으로 나눠 문장 목록에 넣고 선택 · **마침표 뒤 칸이 없어도** 가름 (`A.B.` → `A.` / `B.`) · `3.14` 같은 소수점은 한 덩어리 · 같은 영어 문장이 단원에 있으면 그걸 고름 · 다시 누르면 이전에 만든 `sent-passage-…` 문장만 교체 (이동 없음) |
...
| 단어/본문/문법 추가 팝업 · 취소 / × / Esc | 팝업 닫힘 · 저장 안 함 (이동 없음) |
...
| 3 | `problem-classes` ② 받는 반 | 반을 하나 이상 고르고 「다음」 |
| 4 | `problem-types` ③ 문제 구성 | 유형을 하나 이상 고르고 「다음」 |
| 5 | `problem-split` 「분할하기」 | **「다음」** (설명만 — 안 나눠도 됨 · 나눠 내면 반 홈 「이어서 내기」) |
| 6 | `problem-submit` **제출 버튼 줄** | **둘 중 하나를 누른다** — 「제출하기」(날짜 고르기) 또는 「빠른 제출하기」 |
