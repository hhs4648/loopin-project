...
| **로그인** 탭 | 이메일·비밀번호 폼 (이동 없음) |
| **회원가입** 탭 | 이름·이메일·비밀번호 폼 (이동 없음) |
| **데모 로그인** | 회원가입 없이 `/teacher` · 서버 있으면 익명 세션, 없으면 이 브라우저만 |
| **로그인** 제출 | 성공 → `/teacher` · 실패 → 빨간 안내(아이콘+문구) |
| **계정 만들기** 제출 | 성공 → `/teacher` · 이메일 인증이 필요하면 안내 후 로그인 탭 |
